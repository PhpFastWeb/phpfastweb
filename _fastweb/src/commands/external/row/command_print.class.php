...
class command_print extends acommand_row implements icommand_row {
	...
	function print_print() {
        ...
        foreach ($this->commands as $k => $command) {
            if ($command instanceof icommand_row) {
                $command->set_key_set($this->table->key_set);
            }
            $target = $command->get_execute_url();
            $onclick = $command->get_execute_onclick();
            $label = $command->get_name();
            $result .= "<input type=\"button\" class=\"no_print\" onclick=\"javascript:$onclick\" value=\"    $label    \" />\r\n";            
        }
		$result .= "</div>";
		//$result .= "</td></tr></table>\r\n";
		$result .= "</div>\r\n";
	...
	public function get_row_table($row,$title='') {
	  ...
	  foreach($this->table->columns_col as $col) {
	  	if ( ! ($col instanceof column_hidden) && $col->get_visible() && $col->get_formatted_value() != "") {
